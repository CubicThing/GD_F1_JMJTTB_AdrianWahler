...
BEGIN
 ...
 if Key = 68 then
  ...
  BEGIN
    progressbar1.position := progressbar1.Position - 900;
  ...
  BEGIN
    progressbar2.position := progressbar2.Position - 900;
  ...
  BEGIN
    progressbar3.position := progressbar3.Position - 900;
...
if (progressbar4.position > 0) AND (player.Left >= SValve4.Left) AND (player.Left <= SValve4.Left+65) AND (player.Top >= SValve4.Top) AND (player.Top <= SValve4.Top+65) then
  BEGIN
    progressbar4.position := progressbar4.Position - 900;
...
if (progressbar5.position > 0) AND (player.Left >= SValve5.Left) AND (player.Left <= SValve5.Left+65) AND (player.Top >= SValve5.Top) AND (player.Top <= SValve5.Top+65) then
  BEGIN
    progressbar5.position := progressbar5.position - 900;
...
if key = VK_Space then
if (progressbar6.position > 0) AND (player.Left >= SValve6.Left) AND (player.Left <= SValve6.Left+65) AND (player.Top >= SValve6.Top) AND (player.Top <= SValve6.Top+65) then
  BEGIN
    progressbar6.position := progressbar6.position - 900;
    if progressbar6.Position <= 0 then
    begin
      valve6 := false;
      PlayerTotalScore := PlayerTotalScore + 100;
    end;
...
if valve6 = true then
  ProgressBar6.StepIt;
    if progressbar6.Position > progressBar6.Max - 10 then
  BEGIN
    Valve6 := FALSE;
    Time := Time - 100;
